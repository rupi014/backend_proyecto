...
from schemas import OrdersData
...
async def create_order(order: OrdersData, db: Session = Depends(get_db)):
...
async def update_order(order_id: int, order: OrdersData, db: Session = Depends(get_db)):
