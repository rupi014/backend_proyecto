...
async def create_product(product: ProductData, db: Session = Depends(get_db)):
...
async def update_product(product_id: int, product: ProductData, db: Session = Depends(get_db)):
